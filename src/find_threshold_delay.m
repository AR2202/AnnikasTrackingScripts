...
function find_threshold_delay(outputtable, ...
    outputdir, fullfigname, expname, inputdir, olddataformat)
...
outputvar4 = arrayfun(@(input) input, outputtable.Var4, 'UniformOutput', false);
...
for p = 1:numel(dirnames)
    ...
    filenames = cellfun(@(videoname, var3, var2) strcat(string(videoname), ...
     ...
     filenames_other = cellfun(@(videoname, var3, var4) strcat(string(videoname), ...
        string(var3), string(var4), string(var3)), videonames, ...
        outputtable.Var3, outputvar4, 'UniformOutput', false);
    videonames = videonames(~cellfun(@isempty, videonames));
    filenames = filenames(~cellfun(@isempty, filenames));
    filenames_other = filenames_other(~cellfun(@isempty, filenames_other));
    if size(videonames, 2) > 0
        for q = 1:size(videonames, 1)
            if exist(videonames{q}{1}, 'dir')
                ...
                strofother = filenames_other{q};
                disp(strtofind);
                disp(strofother);
                datafile = dir(char(strcat('*', strtofind, 'threshold.mat')));
                datafile2 = dir(char(strcat('*', strofother, 'threshold.mat')));
                if (size(datafile) > 0) 
                    datafilename = datafile.name;
                    datafilename2 = datafile2.name;
                    disp(datafilename);
                    disp(datafilename2);

                    load(datafilename);
                    dat(end+1) = data.threshold;
                    load(datafilename2);
                    dat2(end+1) = data.threshold;
                end
...
delta_t = dat2 - dat;
meandata = mean(delta_t);
dataSEM = std(delta_t) / sqrt(size(delta_t, 2));
disp(dataSEM);
data_n = size(delta_t, 2);
disp(data_n);

figuredata.mean = meandata;
figuredata.SEM = dataSEM;
figuredata.n = data_n;
figuredata.data = delta_t;

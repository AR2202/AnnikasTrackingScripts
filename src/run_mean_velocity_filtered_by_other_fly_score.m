%Annika Rings, 2022
%RUN_MEAN_VELOCITY_FILTERED_BY_OTHER_FLY_SCORE(score, windowsize, cutofffrac)
...
function run_mean_velocity_filtered_by_other_fly_score(score, windowsize,...
    cutofffrac, varargin)
...
if specificframes %not yet supported
    run_any('mean_velocity_filtered_by_other_fly_errors.log',...
        'mean_velocity_filtered_by_score_other_frames',  dur)
else
run_any('mean_velocity_filtered_by_other_fly_errors.log',...
    'mean_velocity_filtered_by_score_other', score, windowsize, cutofffrac)
end
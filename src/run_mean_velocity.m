%Annika Rings, 2022
%RUN_MEAN_VELOCITY(dur)
%dur: period of time to calculate velocity,
%(in s)
%runs the mean_velocity function for all directories ending in
%'Courtship'
...
function run_mean_velocity(dur, varargin)
...
if specificframes %not yet supported
    run_any('mean_velocity_errors.log', 'mean_velocity_frames',  dur)
else
run_any('mean_velocity_errors.log', 'mean_velocity',    dur)
end
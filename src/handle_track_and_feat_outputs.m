function [wing_ext_frames_indexed] = handle_track_and_feat_outputs(inputfilename)
...
ind_data = arrayfun(@(x, x2) horzcat(transpose(trk.data(x, :, 1)),...
    transpose(trk.data(x, :, 2)), transpose(trk.data(x, :, 3)), ...
    transpose(trk.data(x2, :, 1)), transpose(trk.data(x2, :, 2)), ...
    transpose(feat.data(x, :, 4)), transpose(trk.data(x2, :, 3)), ...
    transpose(feat.data(x, :, 8)), transpose(feat.data(x, :, 9)), ...
    transpose(feat.data(x, :, 10)), transpose(trk.data(x, :, 11)), ...
    transpose(feat.data(x, :, 12)), transpose(feat.data(x2, :, 12)), ...
    transpose(trk.data(x, :, 14)), transpose(trk.data(x, :, 15)), ...
    transpose(trk.data(x, :, 16))), indices, ind2, 'UniformOutput', false);
%ind_data contains:
%  pos x=ind_data(:,:,1);
%  pos y=ind_data(:,:,2);
%ori = ind_data(:,:,3);
...
%ori target fly = ind_data(:,:,9);
%dist_to_other=ind_data(:,:,10);
%facing_angle=ind_data(:,:,12);
%facing_angle target fly=ind_data(:,:,13);
% left_wing_ang=ind_data(:,:,14);
% right_wing_ang=ind_data(:,:,16);
% wing extension
wing_ext_frames_right = cellfun(@(indiv) indiv(indiv(:, 6) > pi / 6, :), ind_data, 'UniformOutput', false);
...
% wing extension frames with copulation removed
...
wing_ext_frames_indexed = cellfun(@(cell1, cell2) {cell1, cell2}, removed_copulation_r, num2cell(indices), 'UniformOutput', false);
